...
(function() {
	...
	function lookForBrowser(browser, output) {
        var testBrowserVersion = new RegExp(browser[0] + ".*: Executed");
        var missing = !testBrowserVersion.test(output);

        var testBrowserOS = new RegExp("\\(" + browser[1] + ".*: Executed");
        missing = missing || !testBrowserOS.test(output);

		if (missing) console.log(browser[0] + " on " + browser[1] + " was not tested!");
		return missing;
	}
